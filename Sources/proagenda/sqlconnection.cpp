...
QList<TaskModel*>SqlConnection::getAllTasks(){
    ...
    while(myQuery.next()){
        TaskModel* t = new TaskModel(myQuery.value(fieldName).toString(), myQuery.value(fieldDesc).toString(),myQuery.value(fieldTermDate).toDate());
...
void SqlConnection::modifyTask(TaskModel* task){
    int rc;
    char *zErrMsg = 0;
